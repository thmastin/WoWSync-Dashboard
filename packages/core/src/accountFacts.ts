...
export interface CharacterProfessionEntry {
  ...
  tier?: string;
  /** Retail expansion name when the export provided it (may be Unknown). */
  expansion?: string;
  category?: string;
}
  ...
  return characters.map((c) => {
    ...
    return {
      ...
      professions: (section?.entries ?? []).map((e) => {
        ...
        if (e.expansion !== undefined) entry.expansion = e.expansion;
...
function buildProfessionCoverage(
  ...
  for (const cp of byCharacter) {
    for (const prof of cp.professions) {
      ...
      const row: { identityKey: string; name: string; skill?: number; maxSkill?: number; tier?: string; expansion?: string; category?: string } = {
      ...
      if (prof.expansion !== undefined) row.expansion = prof.expansion;
