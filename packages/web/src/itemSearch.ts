...
function storageStateFor(character: CharacterFacts | undefined, storage: StorageLocation): "OBSERVED" | "LAST_SEEN" | "UNKNOWN" {
...
export function filterItemRows(rows: readonly ItemSearchRow[], query: string): ItemSearchRow[] {
...
export function searchItemRows(
  ...
  const matched = sortItemRows(filterItemRows(flattenInventoryRows(inventory, characters), query));
...
export function filterInventoryAggregates(items: readonly InventoryAggregateEntry[], query: string): InventoryAggregateEntry[] {
  const q = query.trim().toLowerCase();
  if (q.length === 0) return [];
